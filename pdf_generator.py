...
class ShortlistPDF(FPDF):
    ...
    def header(self):
        ...
        self.cell(0, 15, '🎓 JEE Seat Finder - My Shortlist', 0, 1, 'C')
    ...
    def add_statistics_section(self, df):
        ...
        # Rank statistics
        self.cell(0, 6, f'• Average Closing Rank: {int(avg_rank):,}', 0, 1, 'L')
        self.cell(0, 6, f'• Best Rank (Lowest): {int(min_rank):,}', 0, 1, 'L')
        self.cell(0, 6, f'• Highest Rank: {int(max_rank):,}', 0, 1, 'L')
        self.ln(3)
        ...
        for institute, count in institute_counts.items():
            self.cell(0, 5, f'  • {institute}: {count} option{"s" if count > 1 else ""}', 0, 1, 'L')
        ...
        for seat_type, count in seat_type_counts.items():
            self.cell(0, 5, f'  • {seat_type}: {count} option{"s" if count > 1 else ""}', 0, 1, 'L')
    ...
    try:
        ...
        if filters_applied:
            ...
            for filter_name, filter_value in filters_applied.items():
                if filter_value:
                    pdf.cell(0, 5, f'• {filter_name}: {filter_value}', 0, 1, 'L')
